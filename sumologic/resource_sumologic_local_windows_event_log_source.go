...
func resourceToLocalWindowsEventLogSource(d *schema.ResourceData) LocalWindowsEventLogSource {
	...
	// 	allowlistItems := allowlist.([]interface{})
	// 	allowlistStrings := make([]string, len(allowlistItems))
	// 	for i, v := range allowlistItems {
	// 		allowlistStrings[i] = v.(string)
	// 	}
	// 	source.Allowlist = allowlistStrings
	// }

	// // Handle denylist
	// if denylist, ok := d.GetOk("denylist"); ok {
	// 	denylistItems := denylist.([]interface{})
	// 	denylistStrings := make([]string, len(denylistItems))
	// 	for i, v := range denylistItems {
	// 		denylistStrings[i] = v.(string)
	// 	}
	// 	source.Denylist = denylistStrings
	// }
...
func resourceSumologicLocalWindowsEventLogSourceRead(d *schema.ResourceData, meta interface{}) error {
	...
	if source.EventMessage != nil {
		d.Set("event_message", *source.EventMessage)
	} else {
		d.Set("event_message", nil)
	}
